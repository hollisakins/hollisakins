...
The \gcco is an astronomical observatory used primarily by students for research and as a tool for learning physics. It is operated by the Physics department at Guilford College, 
...
The facility houses several 8-inch Meade telescopes, a 10-inch telescope for spectroscopy, and a 16-inch telescope for optical CCD imaging. 
The 10-inch and 16-inch scopes are permanently mounted, and the 16-inch is remotely controllable. 
Much of the student work done at the observatory uses this 16-inch telescope, 

\section{Project Goals}

\newthought{There are three} primary methods for astronomers to look at the sky: direct imaging, spectroscopy, and photometry. Direct imaging and spectroscopy tools are available at our observatory for students to use and apply to their projects as needed; however, photometry has remained a slow and tedious process. 

The primary goal for this project was to develop a system in which we could perform more thorough and sophisticated photometric observations in research projects. Photometric automation would allow us to more easily study the photometric variations in stars and the systematic limitations of our observatory. Furthermore, these tools would allow future students to work with data gathered over long periods, 
...
All of the software detailed in this section lives on a Linux server and runs once a day on each image captured the previous night. The entirety of the data analysis pipeline was written in Python. 
...
Each image that is captured is calibrated if and only if it is deemed worthwhile---a determination that is made based several factors including image size and sensor temperature at capture.
...
\newthought{Perhaps the most} important step in the pipeline is the photometric measurements. 
...
Phrased slightly differently, it uses every star in the image to calculate a {\it frame zero-point}, to which every instrumental magnitude is adjusted. 
...
The pipeline relies on the user to plate solve each image they want to be processed---this simplifies our program and allows the user control over which images go through the photometry process. 
For example, a user capturing images of a distant galaxy likely wouldn't have any need for photometric data---they also would be unlikely to plate solve their images manually. 

We use the image link feature built in to TheSkyX Pro to find an astrometric solution for our images. 
...
The aperture radius is fixed at 30 pixels for binning 1x1, and decreases as the product of the x and y binning factors. 
...
For each star in the image, we subtract out the background and sum the pixel values in the aperture to generate a flux value. This is then converted into an instrumental magnitude based on the simple formula
\begin{equation}
